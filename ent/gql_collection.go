...
func (c *CustomerQuery) collectField(ctx context.Context, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		switch field.Name {
		case "username":
			if _, ok := fieldSeen[customer.FieldUsername]; !ok {
				selectedFields = append(selectedFields, customer.FieldUsername)
				fieldSeen[customer.FieldUsername] = struct{}{}
			}
		case "password":
		...
		case "idCard":
			if _, ok := fieldSeen[customer.FieldIDCard]; !ok {
				selectedFields = append(selectedFields, customer.FieldIDCard)
				fieldSeen[customer.FieldIDCard] = struct{}{}
			}
		case "memberCard":
			if _, ok := fieldSeen[customer.FieldMemberCard]; !ok {
				selectedFields = append(selectedFields, customer.FieldMemberCard)
				fieldSeen[customer.FieldMemberCard] = struct{}{}
			}
		case "dateOfBirth":
